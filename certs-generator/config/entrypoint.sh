...
patch_package()
{
    sed -i '' -Ee 's/[[:<:]]grep[[:>:]]/ggrep/g' "${OUTPUT_FILE}" || return 1
    sed -i '' -Ee 's/[[:<:]]sed[[:>:]]/gsed/g' "${OUTPUT_FILE}" || return 1
    sed -i '' -Ee 's/[[:<:]]awk[[:>:]]/gawk/g' "${OUTPUT_FILE}" || return 1
}
...
cp /config/certs.yml /config.yml || exit $?
...
source /$CERT_TOOL -A
nodes_server=$( cert_parseYaml /config.yml | ggrep -E "nodes[_]+server[_]+[0-9]+=" | gsed -e 's/nodes__server__[0-9]=//' | gsed 's/"//g' )
...
chown -f 309:309 /certificates/root-ca-manager.pem
chown -f 309:309 /certificates/root-ca-manager.key

for i in ${node_names[@]}; do
  chown -f 309:309 "/certificates/${i}.pem"
  chown -f 309:309 "/certificates/${i}-key.pem"
done
